var reunionPicker = ( function() {
	...
	reunion.PickerClass = function(options) {
		...
		this.schools = [{
			...
			title : 'Engineering MS/PhD',
			fontSize : 14,
			school_abbr : 'SEAS',
			cohort_prefix : 'gradEn'
		}, {
		...
		for(var i in this.schools) {
			this.column_one.addRow(Ti.UI.createPickerRow({
				title : this.schools[i].title,
		...
		this.column_two = Ti.UI.createPickerColumn({
			width : 75
		});

		for(var i in this.years) {
			this.column_two.addRow(Ti.UI.createPickerRow({
				title : this.years[i]
			}));
		}
